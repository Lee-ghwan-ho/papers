...
| ★★★ | **TSIAA** | **Run #8 신규** Teacher-Student Instance-Level Adversarial Augmentation for SSDG (IEEE TMI 2026, Vol 45 pp 764-776). **Instance-level Image Augmenter (IIAG)**: learnable constrained Bézier transformation 기반 IAM으로 per-image 비선형 appearance 변형. Teacher-student adversarial 학습으로 out-of-source 분포 탐색. 내 방법과 공유하는 요소: Bézier nonlinear aug + SSDG. **핵심 차이**: TSIAA = image-level 균일 적용, 나 = intra-image vessel radius별 연속 strength. novelty 구분 필수. |
| ★★★ | **PCSDG** | **Run #8 신규** Structure-Aware Single-Source Generalization with Pixel-Level Disentanglement for Joint Optic Disc and Cup Segmentation (Biomedical Signal Processing and Control 2025). **SABA**: pixel grayscale 값 기반 brightness curve로 augmentation을 구조 특성에 따라 차별화. "structure-aware brightness augmentation"이라는 개념이 내 ONA와 가장 근접한 선행 연구. **핵심 차이**: SABA = pixel intensity 기반 brightness (geometry 무관), 나 = vessel radius/observability 기반 nonlinear appearance. 반드시 구분 논거 작성 필요. |
...
| ★★ | **WAVEADG** | **Run #8 신규** Decoupling Wavelet Sub-bands for SSDG in Fundus Image Segmentation (arXiv 2603.28463, April 2026). WISER module: low-freq sub-band → anatomy anchor, high-freq sub-band → edge enhancement. 1→5 도메인 optic disc/cup SSDG, 7개 SOTA 능가. 내 방법과 차이: frequency-domain 구조/스타일 분리 vs. 나의 spatial-domain vessel radius-conditioned aug. |
| ★★ | **TOPOVST** | **Run #8 신규** TopoVST: Toward Topology-fidelitous Vessel Skeleton Tracking (arXiv 2603.14909, March 2026). Multi-scale sphere graph + GNN으로 vessel tracking direction + **vessel radius** 동시 추정. Wave-propagation skeleton tracking + geometry-aware weighting. vessel radius 추정 방법론이 내 observability score 계산에 직접 활용 가능. |
