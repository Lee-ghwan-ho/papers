...
| ★★★ | **TSIAA** | **Run #8 신규** Teacher-Student Instance-Level Adversarial Augmentation for Single Domain Generalized Medical Image Segmentation (IEEE TMI Vol.45 pp.764-776, 2026). **최우선 novelty 위협**: IIAG의 Instance-level Augmentation Modules(IAMs)가 segment region마다 다른 learnable Bézier curve를 적용 → "intra-image 구조마다 다른 augmentation 강도"를 adversarial 방향으로 구현. 내 ONA와 방향이 같지만 목적이 반대 (adversarial=harder vs. protective=fragile). 구분 논거: (1) TSIAA=더 어렵게, 나=얇은 혈관 보호. (2) region binary vs. radius-continuous. (3) label-image inconsistency 문제의식 없음. 즉시 full text 독해 필수. |
...
| ★★ | **MAMBA_SEA** | **Run #8 신규** Mamba-Sea: Mamba-based Framework with Global-to-Local Sequence Augmentation for Generalizable Medical Image Segmentation (IEEE TMI 2025, arXiv 2504.17515). Global Augmentation(이미지 전체 style 다양화) + Local Sequence Augmentation(Mamba 입력 sub-sequence 단위 token style resampling). 최초 Mamba 기반 SSDG, Prostate Dice 90%+ SOTA. 동일 benchmark 경쟁 방법 → 내 방법과 비교 필요. feature-sequence-level uniform aug vs. 내 input-space structure-conditioned aug. |
...
| ★ | **GRAPHMORPH** | **Run #8 신규** GraphMorph: Tubular Structure Extraction by Morphing Predicted Graphs. NeurIPS 2024, arXiv 2502.11731. Branch-level graph decoder + SkeletonDijkstra morph module. 혈관 + 도로 tubular 구조. Cat C 핵심 NeurIPS 2024 논문. |
| ★ | **WAVESDG** | **Run #8 신규** Decoupling Wavelet Sub-bands for Single Source Domain Generalization in Fundus Image Segmentation. arXiv 2603.28463, April 2026. WaveSDG + WISER module (LL=structure, LH/HL/HH=edge+noise 분리). Cat A 경쟁 논문. Preprint. |
