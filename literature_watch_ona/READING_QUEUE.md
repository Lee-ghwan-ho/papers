...
| ★★ | **WAVESDG** | **Run #8 신규** Decoupling Wavelet Sub-bands for Single Source Domain Generalization in Fundus Image Segmentation (arXiv:2603.28463, April 2026). WISER 모듈: 각 wavelet sub-band의 의미 역할을 활용하여 해부학적 구조와 domain-specific appearance를 분리. Low-freq → 전역 해부 구조 anchor, high-freq → 방향성 엣지 선택적 강화 + 노이즈 억제. 1 source → 5 unseen target optic disc/cup segmentation에서 7 SOTA 능가. 내 방법과의 차이: WaveSDG = frequency-domain 구조-스타일 분리, 나 = intra-image vessel radius별 augmentation budget. 직접 경쟁 관계 파악 필요. |
| ★★ | **DGTAA** | **Run #8 신규** DG-TTA: Out-of-Domain Medical Image Segmentation (Sensors 2025, Vol. 25(17), arXiv:2312.06275). SSC descriptor + GIN intensity augmentation으로 DG pre-training → TTA로 test-time 적응. 5개 CT/MRI 데이터셋. 내 baseline의 GIN 기반 uniform nonlinear aug 비교에 참고 가능. TTA 필요 (target 데이터 필요)하여 pure SSDG 설정과 다름. |
...
| ★ | **VASOMIM** | **Run #8 신규** VasoMIM: Vascular Anatomy-Aware Masked Image Modeling for Vessel Segmentation (AAAI 2026, arXiv:2508.10794). Anatomy-guided masking strategy: vessel patch를 우선 마스킹 → 강력한 혈관 representation 학습. Anatomical consistency loss: 원본-재구성 이미지 간 혈관 구조 일관성 강화. 3 데이터셋에서 SOTA. 내 방법과는 paradigm 다름 (self-supervised pre-training vs. SSDG augmentation). 혈관 특화 학습의 anatomy-guided design 참고 가능. |
| ★ | **BREAKDATABARRIER** | **Run #8 신규** Breaking the Data Barrier: Robust Few-Shot 3D Vessel Segmentation using Foundation Models (arXiv:2602.23782, Feb 2026). DINOv3 기반 foundation model + lightweight 3D Adapter + multi-scale 3D Aggregator로 volumetric vessel 분할. TopCoW (in-domain) + Lausanne (OOD) 실험. 5개 샘플 few-shot에서 nnUNet 대비 +30% relative improvement. OOD 설정에서 +50% improvement. TOF-MRA CoW 관련 데이터셋 사용 → 내 연구와 데이터 겹침. |
| ★ | **UNIVG** | **Run #8 신규** UniVG: Generative Data-engine Foundation Model for Universal Few-shot 2D Vascular Image Segmentation (arXiv:2604.10737, April 2026). Compositionality of vascular images + generative foundation model. Southeast University/University of Macau. 내 방법과는 few-shot foundation vs. SSDG aug이므로 paradigm 다름. 혈관 이미지의 compositionality 모델링 아이디어 참고 가능. |
