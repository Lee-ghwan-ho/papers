...
| ★★★ | **TSIAA** | **Run #8 신규** ⚠️ Teacher-Student Instance-Level Adversarial Augmentation for SDGMIS (IEEE TMI 2026, Vol. 45, pp. 764-776, DOI: 10.1109/TMI.2025.3605162). Instance-level Image Augmenter (IIAG)를 learnable constrained Bézier transformation으로 구성. **"breaks the uniformity of augmentation rules across different structures within an image"** 라고 명시 — 내 ONA의 핵심 주장과 직접 겹치는 표현! 차이: TSIAA = semantic instance 단위(서로 다른 anatomical object들), 나 = 동일 혈관 class 내 local radius별 continuous 조절. 즉시 full text 확인 및 차이 논거 확립 필수. |
...
| ★★ | **WAVESDG** | **Run #8 신규** Decoupling Wavelet Sub-bands for SSDG in Fundus Image Segmentation (arXiv 2603.28463, April 2026, ISI / Abhirup Banerjee et al.). WISER module: wavelet sub-band마다 semantic 역할을 분리(저주파=anatomy, 고주파=style)하여 appearance-structure decoupling. 내 방법과 paradigm 다름(wavelet frequency decomp vs. spatial radius-based budget) — 단, "structure-style decoupling" 목적은 겹침. baseline 비교 가능성 확인 필요. |
| ★★ | **SEGMOTE** | **Run #8 신규** SegMoTE: Token-Level Mixture of Experts for Medical Image Segmentation (CVPR 2026 Oral, arXiv 2602.19213). SAM 기반 token-level MoE: domain-adaptive expertise로 data-efficient + cross-domain 일반화. MedSeg-HQ (소규모) 학습 후 zero-shot 일반화. 내 방법과 paradigm 완전히 다름 (foundation model fine-tuning vs. SSDG augmentation) 하지만 같은 "generalizable medical segmentation" 문제 공간의 CVPR 2026 최신 경쟁. |
| ★★ | **DUALADAPTER_CURV** | **Run #8 신규** Dual-level Adapter Boosting Prompt-free Curvilinear Structure Segmentation (CVPR 2026 Oral). 혈관, 신경 등 curvilinear 구조 분할을 위한 dual-level adapter. arXiv 미공개. 내 Cat C 연구와 직접 연관된 CVPR 2026 Oral — 즉시 conference proceedings 검색 필요. |
