...
| ★★★ | **TSIAA** | **Run #8 신규** Teacher-Student Instance-Level Adversarial Augmentation (IEEE TMI 2026). Instance-level Image Augmenter가 learnable constrained Bézier transform을 **이미지 내 서로 다른 구조에 non-uniform하게** 적용 — SLAug류의 "uniform augmentation" 가정을 명시적으로 깨뜨림. 두 개의 독립 search agent가 모두 "가장 가까운 경쟁 논문"으로 지목. **즉시 전문 독해 필요**: instance-level 단위가 이미지 전체/영역/픽셀 중 무엇인지, radius 등 continuous anatomical signal을 쓰는지 확인. 현재 판단으로는 adversarial min-max 학습(model-driven)이지 vessel radius 같은 해부학적 continuous signal(data-driven) 기반이 아님 — 이 구분이 내 novelty의 핵심 방어선. |
...
| ★★ | **ADVERIN** | **Run #8 신규** AdverIN: Monotonic Adversarial Intensity Attack for DG (Medical Image Analysis 2025; 원 아이디어는 arXiv 2304.02720, 2023). Adversarial하게 학습된 monotonic intensity mapping + spatial mask로 다양한 intensity style 생성. 내 nonlinear/monotonic appearance transform family와 mechanism이 근접 — monotonicity 제약을 공유. 차이: AdverIN은 image-level adversarial 최적화(강도가 구조 관찰가능성에 continuous하게 묶여있지 않음), 나는 source annotation 기반 local radius로 강도를 direct하게 결정. Baseline 비교 후보로 유력. |
| ★★ | **CSWINUNETR** | **Run #8 신규** CSWinUNETR: Segmentation of Thin Anatomical Structures (MICCAI 2026 accepted, arXiv 2606.19824). Cross-shaped stripe self-attention + detail-enhanced multi-scale attention으로 얇고 굴곡진 구조(retinal vessel, cerebral vasculature 포함) 분할. DG 논문은 아니지만 "thin structure의 낮은 contrast/class imbalance로 인한 fragmentation" 문제의식이 내 동기와 직접 연결. Architecture-only contribution이라 내 augmentation-level 방법과 직교(orthogonal), 향후 backbone 후보로 고려 가능. |
| ★★ | **TOPOVST** | **Run #8 신규** TopoVST: Toward Topology-fidelitous Vessel Skeleton Tracking (arXiv 2603.14909, 2026). GNN이 매 tracking step마다 방향과 **local vessel radius를 함께 예측**하며 wave-propagation 방식으로 skeleton을 추출. 내 ONA의 observability score 계산 방식(local radius 추정)과 직접 비교 가능한 최신 radius 추정 기법. DG 논문은 아님. |
...
| ★ | **WAVE_SDG** | **Run #8 신규** WaveSDG: Decoupling Wavelet Sub-bands for SSDG in Fundus Image Segmentation (arXiv 2603.28463, 2026). Wavelet sub-band 분해로 구조/스타일을 분리하는 WISER 모듈. Augmentation이 아닌 architectural feature decoupling 방식. Tubular structure(fundus vessel) SSDG라는 점에서 참고. |
| ★ | **TUBEMLLM** | **Run #8 신규** TubeMLLM: Foundation Model for Topology Knowledge Exploration in Vessel-like Anatomy (arXiv 2603.09217, MICCAI 2026 submission). Topology-critical region에 adaptive loss weighting을 부여하고 zero-shot cross-modality OOD 일반화를 보임. Radius를 augmentation에 쓰지는 않으나 "topology-critical 영역 차등 처리"라는 방향이 내 연구와 유사한 상위 motivation 공유. |
| ★ | **PIXCF_CL** | **Run #8 신규** Pixel-level Counterfactual Contrastive Learning for Medical Image Segmentation (arXiv 2603.17110, 2026, Ben Glocker Lab). Causal generative model(DSCM/HVAE) 기반 counterfactual pixel-level contrastive pretraining으로 scanner/protocol confounder invariance 학습. 내 "label-preserving appearance intervention"과 목표는 유사하나 mechanism(causal generative pretraining vs. structure-conditioned augmentation)이 다름. |
