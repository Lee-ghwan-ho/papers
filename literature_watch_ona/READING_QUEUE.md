...
| ★★★ | **TSIAA** | **Run #8 신규** Teacher-Student Instance-Level Adversarial Augmentation for Single Domain Generalized Medical Image Segmentation (IEEE TMI 2026, DOI: 10.1109/TMI.11146907). **Instance-level Bezier transformation**: image-level augmentation의 uniformity를 깨고 structure instance마다 다른 강도의 변환 적용. "over-augmentation 문제"를 명시적으로 인식하고 instance-level 다양성으로 해결. ⚠️ 내 ONA와 개념 방향 일치 — 구분 논거 즉시 파악 필요. 핵심 차이: TSIAA = adversarial training으로 per-instance 최대 강도 탐색, 나 = vessel radius/observability에 따른 연속적 강도 조절 (thin vessel 보호). |
...
| ★★ | **WAVESDG** | **Run #8 신규** Decoupling Wavelet Sub-bands for Single Source Domain Generalization in Fundus Image Segmentation (arXiv 2603.28463, March 2026). WISER(Wavelet-based Invariant Structure Extraction and Refinement) 모듈: low-freq sub-band으로 global anatomy 앵커, high-freq sub-band으로 directional edge 강화 + noise 억제. optic disc/cup SSDG 5 unseen target. 내 방법과 비교: frequency domain structure preservation vs. 내 spatial radius-conditioned augmentation. |
| ★★ | **TOPOVST** | **Run #8 신규** TopoVST: Toward Topology-fidelitous Vessel Skeleton Tracking (arXiv 2603.14909, March 2026). Multi-scale sphere graph + GNN으로 vessel skeleton 추적 + **vessel radius 동시 추정**. 내 observability score 계산 파이프라인과 직접 연결 가능 — TopoVST의 radius estimation을 inner preprocessing으로 활용 가능성 탐색. Code: https://github.com/EndoluminalSurgicalVision-IMR/TopoVST |
