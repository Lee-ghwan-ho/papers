...
| ★★★ | **TSIAA** | **Run #8 신규 ⚠️ 최우선** Teacher-Student Instance-Level Adversarial Augmentation for SSDG (IEEE TMI 2026, pp.764-776, doc.11146907). **단일 이미지 내 서로 다른 해부학적 구조에 서로 다른 augmentation**을 Instance-level Augmentation Module(IAM)과 Learnable Constrained Bézier Transformation으로 구현. Teacher-student adversarial training. Code: https://github.com/Wangzts0228/TSIAA. 내 Continuous-ONA의 "intra-image structure-specific augmentation" 동기와 가장 근접한 선행연구 — **conditioning signal(adversarial vs. vessel radius), mechanism(adversarial search vs. continuous observability mapping), domain 차이를 명확히 정리하는 것이 핵심 과제**. |
| ★★★ | **CSWINUNETR** | **Run #8 신규** CSWinUNETR: Segmentation of Thin Anatomical Structures (arXiv 2606.19824, June 2026). CSWin Transformer + UNETR 아키텍처. Thin structure의 anisotropic geometry에 특화된 shifted window attention. "Thin structure는 다른 처리가 필요"라는 동기 공유 → 내 augmentation 동기 지지 근거로 활용 가능. |
...
| ★★ | **CCSDG** | **Run #8 신규** Devil is in Channels: Contrastive Single Domain Generalization for Medical Image Segmentation (MICCAI 2023 Early Accept, arXiv 2306.05254). Channel-level feature statistics를 contrastive learning으로 domain-invariant화. Domain-specific vs invariant channel 분리. MICCAI 2023 foundational SSDG 논문 — 기존 인덱스에 없었음. 차이: feature channel 단위 분리 vs. 나의 intra-image spatial radius 조절. |
| ★★ | **DG_DDM_SEG** | **Run #8 신규** Domain-Generalized Discrete Diffusion Model for Cross-Domain Medical Image Segmentation (IEEE TMI April 2025, DOI: 10.1109/TMI.2025.3564474). Discrete diffusion으로 segmentation mask conditional distribution 생성. Two-path reverse diffusion + Robust Feature Extraction Subnet + Mask-Generation Transformer. IEEE TMI 2025 published journal. DG 메커니즘이 생성 모델 기반이라 직접 경쟁은 아니나, TMI 게재 최신 SSDG 기준 논문으로 Related Work에 필수 언급 가능. Code: https://github.com/HeranYang/DG-DDM-Seg |
| ★★ | **PCSDG** | **Run #8 신규** Structure-Aware Single-Source Generalization with Pixel-Level Disentanglement for Optic Disc/Cup Segmentation (BSPC Vol.99, 2025, DOI: 10.1016/j.bspc.2024.106801). SABA(Structure-Aware Brightness Augmentation): disentanglement module으로 content+style map 분리, pixel-wise structure attention map으로 brightness aug 강도 조절. 내 방법과 "pixel-level structural map에 따른 aug 강도 조절"이라는 원칙 공유. 차이: PCSDG = saliency-based binary attention, 나 = vessel radius 기반 연속 observability score. Novelty 위협도 Low-Medium — 상세 확인 필요. |
| ★★ | **SDCL** | **Run #8 신규** Causal Inference via Style Bias Deconfounding for Domain Generalization (IEEE TPAMI 2025, arXiv 2503.16852). SCM으로 style을 confounding factor로 모델링, backdoor adjustment로 style 영향 제거. TPAMI top-tier 출판 논문. 내 방법과 mechanism 다름(causal vs. augmentation) — Related Work "causal DG" 계열 대표 논문으로 인용 가능. |
...
| ★ | **WAVESDG** | **Run #8 신규** Decoupling Wavelet Sub-bands for SSDG in Fundus Image Segmentation (arXiv 2603.28463, April 2026). WISER 모듈: LL sub-band로 global anatomy 보존, high-freq sub-band로 edge 강화. "appearance 변화 동안 structure 보존"이라는 원칙 공유. Fundus 특화, DG 설정이지만 vessel caliber conditioning 없음. 내 motivation section의 frequency-domain 선행연구로 인용 가능. |
| ★ | **TOPBRAIN** | **Run #8 신규** TopBrain: Segmentation Challenge for Whole Brain Vessel Anatomy (medRxiv 2026, DOI: 10.64898/2026.05.28.26354312). 48 vessel classes in MRA+CTA, vessel caliber measurements along centerlines 포함. 내 TOF-MRA 연구 도메인과 직접 인접. **Caliber 분포 데이터가 내 observability score 설계의 외부 검증 근거**로 활용 가능. Dataset/benchmark 논문 — 방법 제안 아님. |
| ★ | **LEARN2SYNTH** | **Run #8 신규** Learn2Synth: Learning Optimal Data Synthesis Using Hypergradients for Brain Image Segmentation (ICCV 2025, arXiv 2411.16719). Bilevel optimization: inner loop segmentation + outer loop synthesis strategy via hypergradient. Brain MRI synthetic data DG. ICCV 2025 top-tier — 합성 데이터 기반 DG의 이론적 체계 참고. 내 방법(real data augmentation)과 패러다임 다름. |
