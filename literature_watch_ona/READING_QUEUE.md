...
| ★★★ | **MRFFD** | **Run #8 신규** Multi-receptive Field Feature Disentanglement + **Distance-Aware Gaussian Brightness Augmentation (DAGBA)** for SSDG in medical image segmentation (Neurocomputing 2025). DAGBA = "distance from pixels to foreground structures and image edges"를 기반으로 Gaussian brightness를 동적으로 조절. **내 방법과의 접점**: "pixel의 구조와의 거리에 따라 augmentation 강도 조절"이라는 아이디어 공유. **핵심 차이**: DAGBA = pixel ↔ foreground 경계 간 거리 기반 global brightness조절; 나 = vessel foreground 내 radius(관찰가능성) 기반 nonlinear appearance perturbation 강도 조절. DAGBA는 thin vessel 보호 목적이 아니며, intra-class vessel 두께 이질성 개념 없음. 즉시 full text 독해 필요. DOI: 10.1016/j.neucom.2025.130120 |
...
| ★★ | **PCSDG** | **Run #8 신규** Structure-Aware Single-Source Generalization with Pixel-Level Disentanglement for Optic Disc and Cup Segmentation (Biomedical Signal Processing and Control 2025). **SABA (Structure-Aware Brightness Augmentation)**: pixel-level contrastive disentanglement + 해부학적 정보 보존하면서 brightness diversity 증폭. **내 방법과의 접점**: "structure-aware brightness aug for SSDG"라는 키워드 직접 공유. **핵심 차이**: PCSDG = optic disc/cup class-level 구조 보존 (organ-class 경계 aware), 나 = vessel foreground 내 연속적 radius별 augmentation budget 조절. GitHub: HopkinsKwong/PCSDG. |
| ★★ | **WAVESDG** | **Run #8 신규** Decoupling Wavelet Sub-bands for SSDG in Fundus Image Segmentation (arXiv 2603.28463, March 2026). WISER module: low-freq sub-band으로 global anatomy 고정, high-freq sub-band으로 directional edges 강화 + noise 억제. Encoder feature-level에서 wavelet 분해 → structure-appearance 분리. Evaluated: optic disc/cup, 1 source + 5 target. **내 방법과의 접점**: "anatomical structure와 appearance를 분리하여 DG 향상" 방향 유사. **핵심 차이**: WaveSDG = wavelet decomposition으로 feature-level 분리, 나 = pixel-level vessel radius conditioned augmentation budget. fundus segmentation에 특화, 혈관 아님. |
