...
| ★★★ | **TSIAA** | **Run #8 신규** Teacher-Student Instance-Level Adversarial Augmentation for SSDG (IEEE TMI 2026, Vol.45 pp.764-776). Learnable constrained Bézier transformation으로 per-sample hard example 생성 (adversarial teacher-student). ADA(MICCAI 2025)와 동일하게 Bézier 기반이나 adversarial training 방식. **핵심 구분점**: TSIAA는 이미지 전체에 단일 augmentation strength → intra-image vessel radius별 조절 없음. 내 "fragile structure보호" 주장과 직접 대비되는 "image-level uniform" 접근. 즉시 전문 독해하여 ADA vs. TSIAA 차이 및 내 방법과의 구분 논거 파악 필수. |
...
| ★★ | **WAVESDG** | **Run #8 신규** Decoupling Wavelet Sub-bands for SSDG in Fundus Image Segmentation (arXiv 2603.28463, March 2026). WISER module: LL sub-band = 해부학적 구조 보존, LH/HL = 방향성 에지, HH = 도메인 노이즈. wavelet decomposition으로 anatomy-appearance 분리. 내 방법과의 차이: WaveSDG = frequency sub-band 분해 후 sub-band별 서로 다른 처리, 나 = spatial domain에서 vessel radius별 augmentation budget 조절. 중요도: Cat A 경쟁 (SSDG, fundus vessel seg). |
| ★★ | **AEGIS** | **Run #8 신규** Aegis: A Domain Generalization Framework for Medical Image Segmentation by Mitigating Feature Misalignment (Pattern Recognition 2025). style aug + DAFC (Dual Attention Feature Calibration) + UFA (uncertainty-guided feature alignment) loss. 3 benchmarks: prostate + fundus + cardiac. Published Journal Article. 내 방법과 유사점: uncertainty guidance, style aug. 차이: Aegis = feature-space alignment constraint, 나 = input-space 공간적 augmentation budget 조절. |
...
| ★ | **CDG** | **Run #8 신규** Continuous Domain Generalization (NeurIPS 2025, arXiv 2505.13519). NeuralLio (Neural Lie Transport Operator): Lie Group theory로 parameter manifold에서의 domain transition을 기하학적으로 연속성 보존. 의료영상 아님 (remote sensing, traffic). 내 "continuous observability conditioned" 개념과 추상적 철학 공명. 수학적 배경 참고용. |
| ★ | **RLAD** | **Run #8 신규** Enhancing Retinal Vessel Segmentation Generalization via Layout-Aware Generative Modelling (arXiv 2503.01190, March 2025). Diffusion model + vascular structure layout conditioning (A/V, OD/OC) → diverse synthetic retinal images → cross-domain DG +8.1%. Preprint Only. 생성 모델 기반 DG 참고용. |
