...
| ★★★ | **TSIAA** | **Run #8 신규** Teacher-Student Instance-Level Adversarial Augmentation for SSDG (IEEE TMI Vol. 45, pp. 764-776, 2026). **Learnable constrained Bézier transformation**으로 per-instance adversarial augmentation을 적용. teacher model이 student를 틀리게 만드는 방향으로 augmentation 강도/형태를 탐색. "over-augmentation 방지" 명시. 내 방법과 "content-adaptive Bézier augmentation" 방향이 가장 가까운 IEEE TMI 2026 논문. **핵심 차이**: TSIAA = per-image (이미지 전체 단위) adversarial aug, 나 = intra-image vessel radius 기반 공간적 augmentation budget 조절. 즉시 full text 독해 필수. |
...
| ★★ | **TOPOVST** | **Run #8 신규** TopoVST: Toward Topology-fidelitous Vessel Skeleton Tracking (arXiv 2603.14909, March 2026). Multi-scale sphere graph + GNN으로 tracking direction과 **vessel radius를 동시 추정**. geometry-aware weighting scheme으로 imbalance 처리. wave-propagation 기반 skeleton tracking. **내 observability score 계산 방식과 유사한 radius estimation approach** → Hessian 기반과 비교하여 참고 가능. |
| ★★ | **AMAP** | **Run #8 신규** Anatomically-Guided Masked Autoencoder with Domain-Adaptive Prompting for Multimodal Cerebral Aneurysm Detection (npj Digital Medicine 2025). TOF-MRA + CTA 멀티모달 cerebrovascular 도메인. anatomically-guided MAE + domain-adaptive prompting + boundary-aware contrastive generalization. 내 TOF-MRA 연구와 동일한 해부학적 도메인(CoW, cerebral arteries). target: aneurysm detection, 나: vessel segmentation DG. 실험 데이터셋 겹칠 수 있음. |
| ★★ | **WAVESDG** | **Run #8 신규** WaveSDG: Decoupling Wavelet Sub-bands for SSDG in Fundus Image Segmentation (arXiv 2603.28463, April 2026). WISER module: LL sub-band(저주파 해부학 구조 고정) + LH/HL/HH sub-band(엣지 방향 선택적 강화)로 domain appearance 분리. SSDG 직접 경쟁, fundus optic disc/cup 평가. 7개 SOTA 능가. 내 방법과의 차이: frequency domain global decomposition vs. 나의 spatial radius-conditioned augmentation. |
| ★★ | **MAMBA_SEA** | **Run #8 신규** Mamba-Sea: A Mamba-based Framework with Global-to-Local Sequence Augmentation for Generalizable Medical Image Segmentation (arXiv 2504.17515, April 2025). Mamba SSM 기반 DG 첫 시도: global sequence augmentation(전체 state space) + local sequence augmentation(local window). SSDG 직접 경쟁. Mamba를 DG에 사용하는 새로운 architecture 방향. 내 augmentation pipeline 기반 접근과 다르지만 SSDG 비교 baseline 후보. |
