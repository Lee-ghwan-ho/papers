...
| ★★★ | **MAMBA_SEA** | **Run #8 신규** Mamba-Sea (IEEE TMI 2025, arXiv 2504.17515). 최초 Mamba 기반 SSDG. Global-to-local sequence augmentation으로 Prostate dataset **90.34% Dice** — 현재 SSDG SOTA. 내 방법의 직접 경쟁 baseline. 차이: architecture-level sequence aug (전체 이미지 token), 나 = intra-vessel radius conditioned appearance. Mamba-Sea에는 thin vessel 보호 없음. 즉시 전문 독해 필요. |
...
| ★★ | **CDSA** | **Run #8 신규** Inter-Class and Inter-Domain Semantic Augmentation for DG (IEEE TIP 2024). CrossSmooth (inter-class semantic direction 샘플링) + CrossVariance (inter-domain style 합성). 자연영상 DG + semantic seg (GTA→Cityscapes) 실험. 내 방법과의 비교: CDSA = inter-class between different categories, 나 = intra-class within same vessel category. semantic direction aug의 최신 IEEE TIP reference. |
