...
| ★★★ | **MRFD_DAGBA** | **Run #8 신규** ⚠️ **novelty 충돌 최우선 확인 필요.** "Distance-Aware Gaussian Brightness Augmentation" — 공간적 거리 정보를 활용한 brightness augmentation for SSDG. "distance"의 정확한 정의가 vessel centerline/radius 기반인지 image-level spatial 기반인지 즉시 확인 필수. Neurocomputing 2025, DOI: 10.1016/j.neucom.2025.130120. |
...
| ★★ | **PCSDG** | **Run #8 신규** Structure-Aware Single-Source Generalization with Pixel-Level Disentanglement for Joint Optic Disc and Cup Segmentation (Biomedical Signal Processing and Control 2025). SABA(Structure-Aware Brightness Augmentation): annotation saliency map으로 structure 영역 vs. background에서 다른 brightness 변환 적용. 내 방법과 유사점: annotation-guided region-specific brightness aug. 핵심 차이: PCSDG는 class-level binary (disc/cup vs. background), 나는 single foreground class 내 연속적 vessel radius별 조절. GitHub: HopkinsKwong/PCSDG. |
| ★★ | **SDCL** | **Run #8 신규** Causal Inference via Style Bias Deconfounding for Domain Generalization (IEEE TPAMI 2025). SCM + backdoor adjustment로 style confounding 제거. Style-Guided Expert Module(SGEM): domain label 없이 style clustering. TPAMI 최고 티어 causal DG 논문. 자연영상 설정. 내 "shortcut suppression" 주장의 이론적 근거로 활용 가능. arXiv: 2503.16852. |
| ★★ | **UNIFREQSDG** | **Run #8 신규** Universal Frequency Domain Perturbation for Single-Source Domain Generalization (ACM Multimedia 2024). LSP(Learnable Spectral Perturbation) + CPR(Content-Preserving Recombination) + ADI(Adaptive Domain Intervention). Fundus +7.47%, Prostate +4.99% over SOTA. 직접 SSDG 경쟁 방법으로 baseline 비교 후보. DOI: 10.1145/3664647.3681536. |
| ★★ | **VESSELSDF** | **Run #8 신규** VesselSDF: Distance Field Priors for Vascular Network Reconstruction (MICCAI 2025, Paper 2121). SDF regression으로 vessel surface까지의 거리를 연속적으로 예측. Adaptive Gaussian regularizer가 vessel surface 근처/먼 영역 다르게 처리. SDF = vessel radius의 implicit encoding → 내 observability measure 계산의 수학적 대안으로 참고 가능. arXiv: 2506.16556. |
