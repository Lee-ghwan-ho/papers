...
| ★★★ | **UNIFREQ** | **Run #8 신규** Universal Frequency Domain Perturbation for SSDG (ACM MM 2024). **Learnable spectral perturbation module**: 주파수 분포 범위를 학습적으로 결정해 hierarchical feature-level frequency perturbation 수행. Active Domain-variance Inducement Loss로 도메인 분리. Fundus +7.47%, Prostate +4.99% vs SOTA. **내 방법과의 차이**: UNIFREQ = 전체 feature map에 균일한 frequency perturbation (no structure conditioning). 나 = intra-image vessel radius별 augmentation budget 연속 조절. 직접 비교 대상. |
| ★★★ | **COLETRA** | **Run #8 신규** Disconnect to Connect: A Data Augmentation Method for Improving Topology Accuracy (arXiv 2503.05541, March 2025, on OpenReview Dec 2025). **핵심 아이디어**: image inpainting으로 tubular structure를 시각적으로 disconnected되게 만들면서 label은 원본 유지 → 모델이 "겉보기에 끊어진 구조도 사실은 연결됨"을 학습. **내 동기와 직접 연결**: CoLeTra가 다루는 "label is connected but appearance is fragmented" = 내가 우려하는 "label은 thin vessel인데 appearance 변형 후 invisible"과 같은 계열의 label-image inconsistency 문제. 차이: CoLeTra는 disconnection augmentation(topology accuracy), 나는 thin vessel의 strong appearance aug 방지(observability protection). |
...
| ★★ | **TOPOFAST** | **Run #8 신규** Topology Optimization in Medical Image Segmentation with Fast Euler Characteristic (IEEE TMI 2025, arXiv 2507.23763). Fast χ computation (2D/3D)으로 topology correctness를 metric으로 평가 + topological violation map 생성. Liu Li, Cheng Ouyang, Johannes Paetzold, Daniel Rueckert, Bernhard Kainz (Imperial College). **내 연구 관련성**: thin vessel 연결성 보존을 loss 또는 augmentation에서 다룰 때 Euler Characteristic이 topology error measurement로 직접 활용 가능. |
| ★★ | **SCSD** | **Run #8 신규** Exploring Semantic Consistency and Style Diversity for Domain Generalized Semantic Segmentation (AAAI 2025, arXiv 2412.12050). Semantic Query Booster + Text-Driven Style Transform으로 자연영상 DGSS에서 semantic consistency와 style diversity를 동시에 최적화. **내 연구 관련성**: "semantic consistency" (구조 내용 유지) + "style diversity" (도메인 변형 증가)의 tension이 내 Continuous-ONA의 핵심 trade-off (thin vessel의 appearance 변형 가능 범위)와 개념적으로 동일. 자연영상이지만 AAAI 2025. |
| ★★ | **VESSELSDF** | **Run #8 신규** VesselSDF: Distance Field Priors for Vascular Network Reconstruction (MICCAI 2025, arXiv 2506.16556). 혈관 분할을 binary voxel classification이 아닌 continuous SDF regression으로 재정의. Distance-weighted regularization이 vessel continuity를 geometric prior로 인코딩 → 일반화 능력 향상. **내 연구 관련성**: vessel의 거리 변환(distance transform)을 활용한 표현이 내 observability score 계산(skeleton distance transform)과 유사한 geometric prior. SDF regression의 generalization 이득이 내 radius-based augmentation budget의 추가 evidence. |
...
| ★ | **URVSM** | **Run #8 신규** Universal Vessel Segmentation for Multi-Modality Retinal Images (IEEE TIP 2025, arXiv 2502.06987, Vol 34 pp 7903-7918). Universal model이 fine-tuning 없이 모든 retinal imaging modality에 대해 혈관 분할 수행. **내 연구 관련성**: modality-agnostic vessel representation을 어떻게 구성하는지 — 내 augmentation이 TOF-MRA 특성에 국한되지 않아야 한다는 점에서 설계 참고. |
| ★ | **WAVESDG** | **Run #8 신규** Decoupling Wavelet Sub-bands for SSDG in Fundus Image Segmentation (arXiv 2603.28463, March 2026, ISI Kolkata/Oxford). WISER module: wavelet sub-band decomposition으로 low-freq(anatomy)와 high-freq(edge/noise)를 분리 처리. Low-freq = global anatomy anchor, high-freq = directional edge enhancement. **내 방법과의 차이**: WaveSDG = 전체 이미지 freq band별 분리, 나 = intra-image vessel radius별 augmentation budget 조절. |
| ★ | **COLORMAP** | **Run #8 신규** Colormap Augmentation: A Novel Method for Cross-Modality Domain Generalization (IJCARS Dec 2025, DOI: 10.1007/s11548-025-03559-y). LUT-based colormap transformation으로 imaging modality를 시뮬레이션 → cross-modality DG를 단순하고 효과적으로 달성. **내 연구 관련성**: 내 baseline augmentation set 비교 시 reference (simple colormap aug vs. structure-conditioned nonlinear aug). |
